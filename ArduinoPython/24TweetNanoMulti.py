...
consumer_key = '7y9lG5G5nMeD6xGessQw2P8G4'
consumer_secret = 'm6PnaLu8ONPxKFZ3IFNLQWX7USLMFg70uCcCPNzgApRFgZw1p0'
...
class StdOutListener(tweepy.StreamListener):
            ...
            def on_data(self, data):
                ...
                tweetPrincipal = '@%s: %s' % (decoded['user']['screen_name'], decoded['text'].encode('ascii', 'ignore'))
                tweetText= decoded['text'].encode('ascii', 'ignore')
                if tweetText != "" :

                    tempoLatido = 0.7
                    tt = threading.Thread(target=latido, args=(tempoLatido,))
                    tt.start()
                    ...
                    if tweetText.find("tierra") != -1: #prender motor 1, defecha 50% PWM
